...
Shader "LiquidSort/BottleLiquid"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // xyz = waterline height, chord centre, chord half width (liquid frame).
            // w = displayed delivery-locked unit. This is explicit metadata, never inferred from RGB.
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                float lockedBoundaryLight = 0.0;
                float lockedFill = saturate(_BandInfo[0].w);
                ...
                for (int k = 0; k < MAX_BANDS - 1; k++)
                {
                    ...
                    float differentColour = smoothstep(0.004, 0.020,
                        colourDistance);
                    float lockedBoundary = saturate(info.w)
                        * saturate(_BandInfo[k + 1].w);
                    boundaryContact = max(boundaryContact,
                        contactBand * differentColour * (1.0 - lockedBoundary));

                    // Locked units share the actual liquid meniscus. A soft blue-grey reflection is strongest
                    // near the walls and weaker across the centre, so it reads as liquid rather than a hard
                    // white divider. Screen derivatives keep it visible on small glasses without changing
                    // either the liquid mask or its geometry during a pour.
                    float lockedEdgeWeight = smoothstep(0.20, 0.88, across);
                    float lockedSoftWidth = max(
                        fwidth(signedBoundary) * lerp(1.30, 1.05, lockedEdgeWeight),
                        chord * 0.004);
                    float lockedDistance = signedBoundary / max(lockedSoftWidth, 1e-5);
                    float lockedSoftProfile = exp2(-2.0 * lockedDistance * lockedDistance);
                    lockedBoundaryLight = max(lockedBoundaryLight,
                        lockedBoundary * lockedSoftProfile
                        * lerp(0.10, 0.46, lockedEdgeWeight));
                    ...
                    lockedFill = lerp(lockedFill,
                        saturate(_BandInfo[k + 1].w), crossed);
                ...
                // A restrained cool key sits under the existing glass reflection and never exposes the
                // concealed palette. Colour-only lighting is clipped by the same liquid mask below.
                float lockedLeftSheen = 1.0 - smoothstep(
                    0.05, 0.45, abs(n.x + 0.52));
                bodyColor = lerp(bodyColor, float3(0.40, 0.54, 0.70),
                    lockedFill * lockedLeftSheen * lockedLeftSheen * 0.045 * bodyOnly);
                bodyColor = lerp(bodyColor, float3(0.48, 0.61, 0.74),
                    saturate(lockedBoundaryLight) * bodyOnly);
