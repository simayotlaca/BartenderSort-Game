...
namespace LiquidSort
{
    ...
    public sealed class VesselFloatingGarnish : MonoBehaviour
    {
        ...
        private void Bind(LiquidBottle owner)
        {
            ...
            ApplyPose(false, default, 0f, 0f, 0f, 0f, 0f, 0f, 0f);
        ...
        private void LateUpdate()
        {
            ...
            bool shouldTrackSurface = orderReady || reserved;

            // Filled vessels also keep their garnish hidden until the order is
            // ready. Once settled, they need no surface solve or renderer writes.
            if ((bottle.DisplayVolume <= 0.05f || !shouldTrackSurface)
                && EmptyMotionIsSettled())
            {
            ...
            Vector2 surfaceCentre = default;
            float halfWidth = 0f;
            bool hasSurface = shouldTrackSurface
                && bottle.DisplayVolume > 0.05f
                && bottle.TryGetContactSurfaceGeometry(
                    out surfaceCentre, out halfWidth, out _);
            ...
            ApplyPose(hasSurface, surfaceCentre, halfWidth,
        ...
        private void ApplyPose(bool hasSurface, Vector2 surfaceCentre,
            ...
            float horizontalDrift, float primaryHeightDelta)
        {
            ...
            SetRendererEnabled(primaryRenderer, visualOpacity > 0.001f);
            if (dual)
            {
                ...
                SetRendererEnabled(secondaryRenderer, visualOpacity > 0.001f);
            }
            else
            {
                HideRenderer(secondaryRenderer);
            ...
            if (primaryRenderer.color != iceColor) primaryRenderer.color = iceColor;
            if (dual && secondaryRenderer.color != iceColor)
                secondaryRenderer.color = iceColor;
            ...
            Vector2 scale, Vector2 centre, float rotationDegrees)
        {
            ...
            Vector3 localScale = new Vector3(scale.x, scale.y, 1f);
            if (!garnishTransform.localScale.Equals(localScale))
                garnishTransform.localScale = localScale;
            Quaternion worldRotation = Quaternion.Euler(
                0f, 0f, rotationDegrees);
            if (!garnishTransform.rotation.Equals(worldRotation))
            ...
            Vector3 worldPosition = visualCenterWorld
                - worldRotation * scaledBoundsCenter;
            if (!garnishTransform.position.Equals(worldPosition))
                garnishTransform.position = worldPosition;
        }
            ...
            float visibility)
        {
            ...
            if (!condensationTransform.localScale.Equals(localScale))
                condensationTransform.localScale = localScale;
            if (!condensationTransform.localRotation.Equals(Quaternion.identity))
            ...
            Vector3 localPosition = new Vector3(
                ...
                -0.01f);
            if (!condensationTransform.localPosition.Equals(localPosition))
                condensationTransform.localPosition = localPosition;
            Color tint = new Color(1f, 1f, 1f,
                visibility * profile.condensationOpacity);
            if (condensationRenderer.color != tint) condensationRenderer.color = tint;
            SetRendererEnabled(condensationRenderer, visibility > 0.001f);
        }
