...
namespace LiquidSort.Levels
{
        ...
        : IEquatable<BartenderDailyOrdersSnapshot>
    {
        ...
        internal string CompactCountdown(long utcNowTicks)
        {
            TimeSpan remaining = RemainingUntilReset(utcNowTicks);
            return remaining.TotalHours >= 1d
                ? string.Format("{0}h {1:00}m", (int)remaining.TotalHours, remaining.Minutes)
                : string.Format("{0}m {1:00}s", remaining.Minutes, remaining.Seconds);
        }
