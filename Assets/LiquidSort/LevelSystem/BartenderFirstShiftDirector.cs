...
namespace LiquidSort.Levels
{
    ...
    public sealed class BartenderFirstShiftDirector : MonoBehaviour, IBartenderInputPolicy
    {
                                ...
                                bool animate = true)
        {
            ...
            {
                overlay.ShowAboveOrders(message, pose, bottle, interaction.InputCamera,
                    FirstVisibleOrderCard(), animate);
                return true;
            }
            return false;
        }

        private RectTransform FirstVisibleOrderCard()
        {
            IReadOnlyList<OrderCardView> cards = orderStrip != null ? orderStrip.Cards : null;
            if (cards == null) return null;
            for (int i = 0; i < cards.Count; i++)
            {
                OrderCardView card = cards[i];
                if (card == null || !card.gameObject.activeInHierarchy) continue;
                RectTransform cardRect = card.Rt != null
                    ? card.Rt
