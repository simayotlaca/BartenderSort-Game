...
namespace LiquidSort.Levels
{
        ...
        IBartenderInputPolicy, IBartenderAcceptedInputConsumer
    {
        ...
        bool IBartenderAcceptedInputConsumer.TryConsume(BartenderInputRequest request)
        {
            ...
            FinishSelection();
            RequestShufflePurchase(glassId, expectedRevision);
            return true;
        }

        private async void RequestShufflePurchase(int glassId, int expectedRevision)
        {
            ...
            try
            {
                BartenderCommandResult<bool> result =
                    await owner.PurchaseShuffleAsync(glassId, expectedRevision);
                if (this == null || !isActiveAndEnabled || generation != presentationGeneration
                    || owner == null || owner.State != BartenderLevelState.Playing
                    || !ReferenceEquals(controller, owner)) return;
                BsRoundCommandStamp current = owner.CurrentRoundStamp;
                if (!stamp.IsValid || !current.IsValid || current.AttemptId != stamp.AttemptId
                    || current.Token != stamp.Token) return;
                if (result.Succeeded) BsAudio.Instance?.Play(BsSfx.ButtonClick);
                else
                {
