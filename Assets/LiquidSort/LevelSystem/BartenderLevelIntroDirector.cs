...
namespace LiquidSort.Levels
{
    ...
    public sealed class BartenderLevelIntroDirector : MonoBehaviour
    {
        ...
        {
            startHasRun = true;
            // A First Shift board is created one frame after this component starts.
            // Let its LevelLoaded event start the entrance once, rather than playing an
            // empty-scene intro and immediately resetting it when the tutorial board arrives.
            if (playOnStart && !levelLoadedSinceEnable
        ...
        private IEnumerator WaitForCover(int version)
        {
            ...
            {
                SnapToRest();
                // The normal canvas pass settles layout during this frame before poses are sampled.
                // Forcing every canvas here also rebuilds the animated loading cover synchronously.
                yield return null;
                if (version != playVersion) yield break;
