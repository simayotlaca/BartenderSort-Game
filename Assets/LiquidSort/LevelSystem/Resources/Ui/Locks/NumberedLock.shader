Shader "LiquidSort/NumberedLock"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _SilverFace ("Blank silver face", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 OutlinedLockFrag(v2f input) : SV_Target
            {
                ...
                // Preserve authored alpha/outline and animated split geometry. Only the interior
                // keyhole is replaced by the generated silver face, never its background.
                float2 uv = input.texcoord;
                float faceMask = (1-smoothstep(0.083,0.103,abs(uv.x-0.5)))
                               * smoothstep(0.223,0.251,uv.y)
                               * (1-smoothstep(0.493,0.516,uv.y));
                fixed3 silver = tex2D(_SilverFace, float2(uv.x,uv.y)).rgb;
                source.rgb = lerp(source.rgb, silver, faceMask);
