...
namespace LiquidSort.Levels
{
    ...
    public sealed class MechanicRevealPresenter : MonoBehaviour
    {
        ...
        private sealed class ActiveFeedback
        {
            ...
            public float FinishFrom;
            public float FinishStartedAt;
        }

        private const string FeedbackChildName = "MechanicRevealFeedback";
        private const float RevealQuickFinishSeconds = 0.14f;
        ...
        private void HandlePoured(BartenderPourReceipt receipt)
        {
            ...
            SettleFeedbackForPour(receipt.SourceBefore != null
                ? receipt.SourceBefore.Id
                : -1);
            SettleFeedbackForPour(receipt.TargetBefore != null
                                                   ...
                                                   int afterDelivered)
        {
            ...
            return GlassLockPresenter.OpensPresentedLock(glass, beforeDelivered, afterDelivered);
        }

        private void Queue(int revision, int glassId, BeatKind kind, Color tint,
                           int unitIndex = -1)
        {
            if (revision < 0 || glassId < 0 || controller == null) return;
            RtGlass glass = FindGlass(controller.Board, glassId);
            ...
            {
                Revision = revision,
                AttemptId = controller.CurrentRoundStamp.AttemptId,
                Token = controller.CurrentRoundStamp.Token,
                GlassSnapshot = glass.Clone(),
                GlassId = glassId,
                Kind = kind,
                Tint = tint,
                UnitIndex = unitIndex
        ...
        private void TryFlushPending()
        {
            if (pending.Count == 0 || controller == null || shelfView == null
                || !shelfView.Ready || shelfView.GlobalSynchronizationDeferred)
                return;
            ...
            for (int i = pending.Count - 1; i >= 0; i--)
            {
                ...
                if (beat.AttemptId != stamp.AttemptId || beat.Token != stamp.Token
                    || !SameGlass(beat.GlassSnapshot, FindGlass(snapshot, beat.GlassId)))
                {
                    pending.RemoveAt(i);
                    continue;
                }
                if (shelfView.IsGlassSynchronizationDeferred(beat.GlassId)) continue;

                if (beat.Kind == BeatKind.LockOpened
                    && (shelfView.BlockingSeatRunPlaying || shelfView.IsSeatMoving(beat.GlassId)))
                    continue;
        ...
        private static RtGlass FindGlass(BsBoard snapshot, int glassId)
        {
            if (snapshot == null) return null;
            for (int i = 0; i < snapshot.Glasses.Count; i++)
                if (snapshot.Glasses[i].Id == glassId) return snapshot.Glasses[i];
            return null;
        }

        private static bool SameGlass(RtGlass expected, RtGlass current)
        {
            if (expected == null || current == null || expected.Id != current.Id
                || expected.Type != current.Type || expected.UnlockAfter != current.UnlockAfter
                || expected.Layers.Count != current.Layers.Count)
                return false;
            for (int i = 0; i < expected.Layers.Count; i++)
                if (!expected.Layers[i].Equals(current.Layers[i])) return false;
            return true;
        }
        ...
        private void PlayRevealTurbulence(LiquidBottle bottle, PendingBeat beat)
        {
            var feedback = new ActiveFeedback
            {
                ...
                AttemptId = beat.AttemptId,
                Token = beat.Token,
                GlassId = beat.GlassId,
        ...
        private void ApplyRevealTurbulence(ActiveFeedback feedback, float progress)
        {
            ...
            if (controller == null
                || controller.CurrentRoundStamp.AttemptId != feedback.AttemptId
                || controller.CurrentRoundStamp.Token != feedback.Token
                || shelfView == null
                || !shelfView.TryGetBottle(feedback.GlassId, out LiquidBottle current)
                || !ReferenceEquals(current, bottle)
                || bottle.ModelVersion != feedback.ModelVersion
                || bottle.IsTransferReserved && !feedback.Finishing)
            {
                StopFeedback(bottle);
                return;
            }

            if (feedback.Finishing)
            {
                float finish = Mathf.Clamp01((Time.unscaledTime - feedback.FinishStartedAt)
                    / RevealQuickFinishSeconds);
                bottle.SetHiddenColorRevealProgress(Mathf.Lerp(feedback.FinishFrom, 1f,
                    Mathf.SmoothStep(0f, 1f, finish)));
                feedback.Shell?.Refresh();
                if (finish >= 1f) FinishFeedback(feedback);
                return;
            }
            ...
            bottle.SetHiddenColorRevealProgress(RevealColorProgress(life));
            feedback.Shell?.Refresh();
            if (feedback.Renderer != null)
            {
        ...
        private static float RevealColorProgress(float life) =>
            Mathf.SmoothStep(0f, 1f, Mathf.InverseLerp(0.10f, 0.78f, life));

        private void SettleFeedbackForPour(int glassId)
        {
            if (glassId < 0 || shelfView == null
                || !shelfView.TryGetBottle(glassId, out LiquidBottle bottle)
                || !active.TryGetValue(bottle, out ActiveFeedback feedback)
                || feedback.Kind != BeatKind.HiddenReveal || feedback.Tween == null)
            {
                StopFeedbackForGlass(glassId);
                return;
            }

            for (int i = pending.Count - 1; i >= 0; i--)
                if (pending[i].GlassId == glassId) pending.RemoveAt(i);
            if (feedback.Finishing) return;

            feedback.Finishing = true;
            feedback.FinishFrom = RevealColorProgress(feedback.Life);
            feedback.FinishStartedAt = Time.unscaledTime;
            bottle.SetRevealTurbulence(0f, feedback.Life);
