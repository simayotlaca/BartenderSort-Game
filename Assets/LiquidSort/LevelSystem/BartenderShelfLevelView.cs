...
namespace LiquidSort.Levels
{
    ...
    public sealed class BartenderShelfLevelView : MonoBehaviour
    {
        ...
        private sealed class SynchronizationDeferral
        {
            public object Owner;
            public BsShelfSynchronizationLease Lease;
            public int SourceGlassId;
            public int TargetGlassId;
            public double Deadline;
            public bool PersistencePending;
            public BsPourOperationStateMachine Lifecycle;
            public BartenderBoardChange LatestChange;
            public bool Global => SourceGlassId < 0;
            public bool Dirty;

            public bool Contains(int glassId) =>
                Global || SourceGlassId == glassId || TargetGlassId == glassId;
        }

        private sealed class DepartureRun
        {
            public BartenderDeliveryReceipt Receipt;
            public Actor Actor;
            public BottleDeliveryEffect Effect;
            public long EffectRunId;
            public float Duration;
            public float Elapsed;
            public double PreviousTick;
            public double CreatedAt;
            public double StartedAt;
            public bool OriginCaptured;
            public Vector3 OriginWorld;
            public Color OriginColor;
            public Bounds OriginBounds;
            public float SampleElapsed;
            public float SampleOpacity = 1f;
            public bool Started;
            public bool Finished;
            public double FinishedAt;
        }

        private sealed class SeatLease
        {
            public object Owner;
            public int GlassId;
            public bool FollowsSeat;
        }

        private sealed class SeatCatchUp
        {
            public Actor Actor;
            public int GlassId;
        ...
        private sealed class Actor
        {
            ...
            public readonly List<Color> AppliedColors = new List<Color>();
            public int AppliedContentsModelVersion = -1;
        ...
        internal sealed class SeatPresentationRun
        {
            ...
            internal bool Blocking { get; set; } = true;
            internal HashSet<int> Excluded { get; } = new HashSet<int>();
        ...
        [Tooltip("Resting order-card anchor.")]
        ...
        [Header("Automatic glass arrangement")]
        [Tooltip("Seats glasses by type so big and small glasses are shared across shelves (ShelfArrangement "
               + "rules) instead of the level's list order. Off keeps the list order.")]
        [SerializeField] private bool arrangeGlassesByType = true;
        [Tooltip("Spaces each shelf by the visible glass outlines so neighbours get equal gaps. "
               + "A shelf never gets wider than the count-based spacing.")]
        [SerializeField] private bool spaceGlassesByOutline = true;
        [Tooltip("How much looser a shelf's gaps may stay than the tightest shelf's gaps.")]
        [SerializeField, Range(1f, 3f)] private float maximumShelfGapRatio = 1.6f;
        ...
        [Tooltip("Slide remaining glasses to their new seats after deliveries and purchases.")]
        [SerializeField] private bool animateEntrance = true;
        [Tooltip("Drop glasses in on level load.")]
        [SerializeField] private bool dropGlassesOnEntrance = false;
        ...
        [Tooltip("Neutral fill used while a liquid unit's colour is hidden.")]
        [SerializeField] private Color hiddenLayerColor = new Color(0.37f, 0.42f, 0.49f, 1f);
        [Tooltip("Darker neutral fill for a delivery-locked unit. Distinct from mystery fill "
               + "so adjacent locked and hidden units keep their visible liquid boundary.")]
        [SerializeField] private Color lockedLayerColor = new Color(0.17f, 0.23f, 0.32f, 1f);
        [Tooltip("Lift for every second unit in a run of locked units, so the liquid seam shows how many "
               + "layers one grouped padlock covers. Soft curved boundary light carries the separation; "
               + "this subtle tone change keeps all units in one navy slate family.")]
        [SerializeField, Range(0f, 0.08f)] private float lockedLayerSeamLift = 0.012f;
        ...
        // Seat -> activeActors index for the current layout. activeActors stays in board order because
        // contents-only syncs match it to the snapshot index by index.
        private readonly ShelfArrangement.Workspace arrangementWorkspace = new ShelfArrangement.Workspace();
        private readonly int[] seatGlassIds = new int[MaximumActiveGlasses];
        private readonly int[] seatOrder = new int[MaximumActiveGlasses];
        private readonly float[] seatVisibleMinX = new float[MaximumActiveGlasses];
        private readonly float[] seatVisibleMaxX = new float[MaximumActiveGlasses];
        private readonly float[] rowEvenGap = new float[ShelfLayoutSolver.MaximumRowCount];
        private int seatOrderCount;
        ...
        private readonly List<SynchronizationDeferral> synchronizationDeferrals =
            new List<SynchronizationDeferral>();
        private long nextSynchronizationLease;
        private SeatPresentationRun activeSeatPresentation;
        private long nextSeatPresentationRunId;
        // Delivered glasses leave input and layout at once but stay reserved in the pool until their own exit
        // settles. Several can dissolve at the same time; each run keeps its own clock and sample.
        private readonly List<DepartureRun> departures = new List<DepartureRun>(4);
        private readonly List<BottleDeliveryEffect> deliveryEffectClones = new List<BottleDeliveryEffect>(2);
        private bool missingDeliveryEffectReported;
        private bool compactionPending;
        private bool startingCompaction;
        private bool keepPosesOnSeatSettle;
        private SeatPresentationRun settlingSeatRun;
        private readonly List<SeatLease> seatLeases = new List<SeatLease>(8);
        private readonly List<SeatCatchUp> seatCatchUps = new List<SeatCatchUp>(2);
        private bool publishingDetachedDelivery;
        private readonly List<Actor> compactionPoseActors = new List<Actor>(4);
        private readonly List<Vector3> compactionPosePositions = new List<Vector3>(4);
        private readonly List<Quaternion> compactionPoseRotations = new List<Quaternion>(4);
        private readonly List<Vector3> compactionPoseScales = new List<Vector3>(4);
        // A departure the order strip asked about before the shelf detached it gives up after this long.
        private const double UnstartedDepartureTimeout = 1d;
        // Finished samples stay readable a little longer so a late order card still finds them.
        private const double FinishedDepartureRetention = 2d;
        private const float CompactionDepartureDelay = 0.10f;
        ...
        public bool BlockingSeatRunPlaying =>
            SeatAnimationPlaying && activeSeatPresentation.Blocking;
        public bool CompactionPending => compactionPending;
        internal bool PresentationCurrent => Ready && controller != null
            && ReferenceEquals(presentedLevel, controller.CurrentLevel)
            && presentedBoardRevision == controller.BoardRevision;
        internal bool PresentationChangeKeptPoses => publishingDetachedDelivery;
        public bool CompactionPlaying =>
            SeatAnimationPlaying && !activeSeatPresentation.Blocking;
        public bool DeparturesPlaying
        {
            get
            {
                for (int i = 0; i < departures.Count; i++)
                    if (!departures[i].Finished) return true;
                return false;
            }
        }
        ...
        public bool SynchronizationDeferred =>
            synchronizationDeferrals.Count > 0;
        public bool GlobalSynchronizationDeferred =>
            synchronizationDeferrals.Count > 0 && synchronizationDeferrals[0].Global;

        public bool IsGlassSynchronizationDeferred(int glassId)
        {
            for (int i = 0; i < synchronizationDeferrals.Count; i++)
                if (synchronizationDeferrals[i].Contains(glassId)) return true;
            return false;
        }
        ...
        {
            for (int i = 0; i < deliveryEffectClones.Count; i++)
            {
                BottleDeliveryEffect clone = deliveryEffectClones[i];
                if (clone != null) Destroy(clone.gameObject);
            }
            deliveryEffectClones.Clear();
        }

        private void LateUpdate()
        {
            try { StepSeatCatchUps(); }
            catch (Exception exception) { Debug.LogException(exception, this); }
            // Delivered glasses dissolve on their own clocks; the order strip samples them later in this frame.
            try { StepDepartures(); }
            catch (Exception exception) { Debug.LogException(exception, this); }
            try { TryStartCompaction(); }
            catch (Exception exception) { Debug.LogException(exception, this); }
            ...
            ExecutePresentationCleanup(
                () => SettleExpiredSynchronizationDeferrals(now),
        ...
        private bool RefreshFromController(BartenderBoardChange change)
        {
            ...
            {
                if (GlobalSynchronizationDeferred) return true;
                return TrySynchronizeAvailableContents(snapshot, controller.Palette,
                    controller.BoardRevision);
            }
            ...
            CoveredPresentationRun coveredRun = null)
        {
            ...
            synchronizationDeferrals.Clear();
            StopSeatAnimation();
            // A new board reuses the pool, so delivered glasses still dissolving finish first.
            FinishAllDepartures();
            ...
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                ...
                if (actor == null)
                    return Reject($"{BsRules.DisplayName(glass.Type)} pool exhausted.");
        ...
        internal bool TryGetDeliveryOrigin(BartenderDeliveryReceipt receipt,
            out Vector3 world, out Color tint, out Bounds bounds)
        {
            world = default;
            tint = default;
            bounds = default;
            if (receipt == null) return false;
            DepartureRun run = FindDeparture(receipt);
            // The order strip can handle the commit before this view detaches the glass; capture it while mapped.
            if (run == null && receipt.DeliveredGlass != null
                && actorByGlassId.TryGetValue(receipt.DeliveredGlass.Id, out Actor actor)
                && actor != null && actor.Bottle != null)
            {
                run = new DepartureRun
                {
                    Receipt = receipt,
                    CreatedAt = Time.realtimeSinceStartupAsDouble,
                };
                CaptureDepartureOrigin(run, actor);
                departures.Add(run);
            }
            if (run == null || !run.OriginCaptured) return false;
            world = run.OriginWorld;
            tint = run.OriginColor;
            bounds = run.OriginBounds;
            return true;
        }

        internal bool TryGetDeliverySample(BartenderDeliveryReceipt receipt,
            out float seconds, out float opacity, out bool finished)
        {
            DepartureRun run = FindDeparture(receipt);
            if (run == null)
            {
                seconds = DeliveryPoofGraphic.DisappearSeconds;
                opacity = 0f;
                finished = true;
                return receipt != null;
            }
            seconds = run.SampleElapsed;
            opacity = run.SampleOpacity;
            finished = run.Finished;
            return true;
        }

        private DepartureRun FindDeparture(BartenderDeliveryReceipt receipt)
        {
            if (receipt == null) return null;
            for (int i = 0; i < departures.Count; i++)
                if (ReferenceEquals(departures[i].Receipt, receipt)) return departures[i];
            return null;
        }

        private static void CaptureDepartureOrigin(DepartureRun run, Actor actor)
        {
            if (run == null || actor?.Bottle == null) return;
            SpriteRenderer placement = actor.PlacementRenderer;
            run.OriginWorld = placement != null
                ? placement.transform.TransformPoint(placement.localBounds.center)
                : actor.Bottle.transform.TransformPoint(actor.Bottle.InteriorBounds.center);
            run.OriginBounds = placement != null ? placement.bounds
                : new Bounds(run.OriginWorld, actor.Bottle.transform.TransformVector(
                    new Vector3(actor.Bottle.InteriorBounds.width, actor.Bottle.InteriorBounds.height, 0f)));
            run.OriginColor = actor.Bottle.VisualTopColor;
            run.OriginCaptured = true;
        }

        private void BeginDeparture(Actor actor, BartenderDeliveryReceipt receipt)
        {
            ...
            {
                run = new DepartureRun { Receipt = receipt, CreatedAt = now };
                departures.Add(run);
            }
            CaptureDepartureOrigin(run, actor);
            // A record the order strip made that timed out before this glass detached is reused: clear its finish,
            // or the run would never be stepped, stopped or released.
            run.Finished = false;
            run.FinishedAt = 0d;
            run.Actor = actor;
            run.Started = true;
            ...
            run.Elapsed = 0f;
            run.SampleElapsed = 0f;
            run.SampleOpacity = 1f;

            BottleDeliveryEffect effect = CanAnimate ? AcquireDeliveryEffect() : null;
            long effectRunId = 0L;
            bool started = false;
            try
            {
                started = effect != null && effect.Begin(actor.Bottle, MotionRoot(actor), GlassSpace,
                    actor.PlacementRenderer, out effectRunId);
            }
            ...
            {
                run.Effect = effect;
                run.EffectRunId = effectRunId;
                run.Duration = Mathf.Max(0.001f, effect.Duration);
                missingDeliveryEffectReported = false;
                effect.Step(0f, effectRunId);
                run.SampleOpacity = effect.SampledOpacity;
                return;
            }
            if (deliveryEffect == null && CanAnimate && !missingDeliveryEffectReported)
            {
        ...
        private BottleDeliveryEffect AcquireDeliveryEffect()
        {
            if (deliveryEffect == null) return null;
            if (!deliveryEffect.IsPlaying && deliveryEffect.isActiveAndEnabled) return deliveryEffect;
            for (int i = deliveryEffectClones.Count - 1; i >= 0; i--)
            {
                BottleDeliveryEffect clone = deliveryEffectClones[i];
                if (clone == null)
                {
                    deliveryEffectClones.RemoveAt(i);
                    continue;
                }
                if (!clone.IsPlaying && clone.isActiveAndEnabled) return clone;
            }
            if (!deliveryEffect.isActiveAndEnabled) return null;
            BottleDeliveryEffect created = Instantiate(deliveryEffect, deliveryEffect.transform.parent);
            created.name = deliveryEffect.name + " (Pooled Departure)";
            // The copy starts idle; it must not show the playing template's sparkle mesh before its own run.
            foreach (MeshRenderer sparkleRenderer in created.GetComponentsInChildren<MeshRenderer>(true))
                sparkleRenderer.enabled = false;
            foreach (MeshFilter sparkleFilter in created.GetComponentsInChildren<MeshFilter>(true))
                sparkleFilter.sharedMesh = null;
            deliveryEffectClones.Add(created);
            return created;
        }

        private void StepDepartures()
        {
            if (departures.Count == 0) return;
            double now = Time.realtimeSinceStartupAsDouble;
            for (int i = departures.Count - 1; i >= 0; i--)
            {
                DepartureRun run = departures[i];
                if (run.Finished)
                {
                    if (now - run.FinishedAt >= FinishedDepartureRetention) departures.RemoveAt(i);
                    continue;
                }
                if (!run.Started)
                {
                    if (now - run.CreatedAt >= UnstartedDepartureTimeout) FinishDeparture(run);
                    continue;
                }
                float maximumStep = Mathf.Min(0.05f, run.Duration * 0.2f);
                run.Elapsed += Mathf.Clamp((float)(now - run.PreviousTick), 0f, maximumStep);
                run.PreviousTick = now;
                BottleDeliveryEffect effect = run.Effect;
                if (effect != null && effect.OwnsRun(run.EffectRunId))
                {
                    effect.Step(run.Elapsed, run.EffectRunId);
                    run.SampleElapsed = run.Elapsed / Mathf.Max(0.001f, run.Duration)
                        * DeliveryPoofGraphic.DisappearSeconds;
                    run.SampleOpacity = effect.SampledOpacity;
                }
                if (run.Elapsed >= run.Duration || effect == null || !effect.OwnsRun(run.EffectRunId))
                    FinishDeparture(run);
            }
        }

        private void FinishDeparture(DepartureRun run)
        {
            if (run == null || run.Finished) return;
            run.Finished = true;
            run.FinishedAt = Time.realtimeSinceStartupAsDouble;
            run.SampleElapsed = DeliveryPoofGraphic.DisappearSeconds;
            run.SampleOpacity = 0f;
            Actor actor = run.Actor;
            BottleDeliveryEffect effect = run.Effect;
            long effectRunId = run.EffectRunId;
            run.Actor = null;
            run.Effect = null;
            run.EffectRunId = 0L;
        ...
        private void FinishAllDepartures()
        {
            Exception failure = null;
            for (int i = 0; i < departures.Count; i++)
            {
                try { FinishDeparture(departures[i]); }
                catch (Exception exception) { if (failure == null) failure = exception; }
            }
            if (failure != null) throw failure;
        }

        private bool IsDepartingActor(Actor actor)
        {
            if (actor == null) return false;
            for (int i = 0; i < departures.Count; i++)
                if (!departures[i].Finished && ReferenceEquals(departures[i].Actor, actor)) return true;
            return false;
        }

        private double YoungestDepartureAge()
        {
            double now = Time.realtimeSinceStartupAsDouble;
            double youngest = double.PositiveInfinity;
            for (int i = 0; i < departures.Count; i++)
            {
                DepartureRun run = departures[i];
                if (run.Started && !run.Finished) youngest = Math.Min(youngest, now - run.StartedAt);
            }
            return youngest;
        }

        internal bool TryAcquireSeatLease(object owner, int glassId, bool followsSeat)
        {
            if (owner == null || glassId < -1) return false;
            for (int i = 0; i < seatLeases.Count; i++)
            {
                if (!ReferenceEquals(seatLeases[i].Owner, owner)) continue;
                seatLeases[i].GlassId = glassId;
                seatLeases[i].FollowsSeat = followsSeat;
                return true;
            }
            seatLeases.Add(new SeatLease { Owner = owner, GlassId = glassId, FollowsSeat = followsSeat });
            return true;
        }

        internal void ReleaseSeatLease(object owner)
        {
            if (owner == null) return;
            for (int i = seatLeases.Count - 1; i >= 0; i--)
            {
                if (!ReferenceEquals(seatLeases[i].Owner, owner)) continue;
                seatLeases.RemoveAt(i);
            }
        }

        private bool HasFollowingSeatLease(int glassId)
        {
            if (glassId < 0) return false;
            for (int i = 0; i < seatLeases.Count; i++)
                if (seatLeases[i].FollowsSeat && seatLeases[i].GlassId == glassId) return true;
            return false;
        }

        private bool HasHoldingSeatLease()
        {
            for (int i = 0; i < seatLeases.Count; i++)
                if (!seatLeases[i].FollowsSeat) return true;
            return false;
        }

        internal bool IsSeatMoving(int glassId)
        {
            if (FindSeatCatchUp(glassId) >= 0) return true;
            SeatPresentationRun run = activeSeatPresentation;
            if (run == null || run.Routine == null
                || !actorByGlassId.TryGetValue(glassId, out Actor actor) || actor == null)
                return false;
            if (run.Blocking) return true;
            return !run.Excluded.Contains(glassId) && SeatChanged(actor);
        }

        private static bool SeatChanged(Actor actor) =>
            actor.HasPreviousSeat
            && ((actor.PreviousLayoutPosition - actor.SeatLayoutPosition).sqrMagnitude > 1e-6f
                || Quaternion.Angle(actor.PreviousLayoutRotation, actor.SeatLayoutRotation) > 0.01f
                || (actor.PreviousSeatScale - actor.SeatScale).sqrMagnitude > 1e-6f);

        internal bool FinishSeatMotion(int glassId)
        {
            SeatPresentationRun run = activeSeatPresentation;
            if (run == null || run.Routine == null || run.Blocking
                || !actorByGlassId.TryGetValue(glassId, out Actor actor)
                || actor == null || actor.Bottle == null || !actor.Seated
                || run.Excluded.Contains(glassId)) return false;
            run.Excluded.Add(glassId);
            if (actor.Bottle.IsTransferReserved) return false;
            Transform actorTransform = MotionRoot(actor);
            int existing = FindSeatCatchUp(glassId);
            if (existing >= 0) seatCatchUps.RemoveAt(existing);
            seatCatchUps.Add(new SeatCatchUp
            {
                Actor = actor,
                GlassId = glassId,
                FromPosition = actorTransform.position,
                FromRotation = actorTransform.rotation,
                FromScale = actorTransform.localScale,
                StartedAt = -1d,
            });
            actorTransform.SetPositionAndRotation(SeatWorldPosition(actor), SeatWorldRotation(actor));
            actorTransform.localScale = actor.SeatScale;
            return true;
        }

        internal void StartSeatCatchUp(int glassId, float seconds)
        {
            int index = FindSeatCatchUp(glassId);
            if (index < 0) return;
            SeatCatchUp catchUp = seatCatchUps[index];
            if (catchUp.StartedAt >= 0d || !IsSeatCatchUpCurrent(catchUp) || !(seconds > 0f))
            {
                seatCatchUps.RemoveAt(index);
                return;
            }
            Transform actorTransform = MotionRoot(catchUp.Actor);
            actorTransform.SetPositionAndRotation(catchUp.FromPosition, catchUp.FromRotation);
            actorTransform.localScale = catchUp.FromScale;
            catchUp.StartedAt = Time.realtimeSinceStartupAsDouble;
            catchUp.Duration = seconds;
        }

        private int FindSeatCatchUp(int glassId)
        {
            if (glassId < 0) return -1;
            for (int i = 0; i < seatCatchUps.Count; i++)
                if (seatCatchUps[i].GlassId == glassId) return i;
            return -1;
        }

        private bool IsSeatCatchUpCurrent(SeatCatchUp catchUp) =>
            catchUp?.Actor?.Bottle != null && catchUp.Actor.Seated
            && actorByGlassId.TryGetValue(catchUp.GlassId, out Actor mapped)
            && ReferenceEquals(mapped, catchUp.Actor);

        private void StepSeatCatchUps()
        {
            if (seatCatchUps.Count == 0) return;
            double now = Time.realtimeSinceStartupAsDouble;
            for (int i = seatCatchUps.Count - 1; i >= 0; i--)
            {
                SeatCatchUp catchUp = seatCatchUps[i];
                if (catchUp.StartedAt < 0d || !IsSeatCatchUpCurrent(catchUp))
                {
                    seatCatchUps.RemoveAt(i);
                    continue;
                }
                float t = Mathf.Clamp01((float)((now - catchUp.StartedAt) / catchUp.Duration));
                float k = 1f - (1f - t) * (1f - t);
                Transform actorTransform = MotionRoot(catchUp.Actor);
                actorTransform.SetPositionAndRotation(
                    Vector3.LerpUnclamped(catchUp.FromPosition, SeatWorldPosition(catchUp.Actor), k),
                    Quaternion.SlerpUnclamped(catchUp.FromRotation, SeatWorldRotation(catchUp.Actor), k));
                actorTransform.localScale = Vector3.LerpUnclamped(catchUp.FromScale, catchUp.Actor.SeatScale, k);
                if (t >= 1f) seatCatchUps.RemoveAt(i);
            }
        }

        private bool SkipsSeatWrite(SeatPresentationRun run, Actor actor)
        {
            if (actor == null || actor.Bottle == null) return true;
            if (run != null && run.Excluded.Contains(actor.GlassId)) return true;
            bool owned = actor.Bottle.IsTransferReserved || HasFollowingSeatLease(actor.GlassId);
            if (owned && run != null) run.Excluded.Add(actor.GlassId);
            return owned;
        }

        private void TryStartCompaction()
        {
            if (!compactionPending || startingCompaction || !Ready || controller == null
                || controller.State != BartenderLevelState.Playing
                || !ReferenceEquals(presentedLevel, controller.CurrentLevel)
                || controller.ModalInputBlocked || controller.CommandWriterBusy
                || controller.PresentationLocked
                || activeSeatPresentation != null || synchronizationDeferrals.Count > 0
                || HasHoldingSeatLease() || seatCatchUps.Count > 0)
                return;
            for (int i = 0; i < activeActors.Count; i++)
            {
                LiquidBottle bottle = activeActors[i]?.Bottle;
                if (bottle != null && bottle.IsTransferReserved) return;
            }

            startingCompaction = true;
            try
            {
                compactionPending = false;
                compactionPoseActors.Clear();
                compactionPosePositions.Clear();
                compactionPoseRotations.Clear();
                compactionPoseScales.Clear();
                for (int i = 0; i < activeActors.Count; i++)
                {
                    Actor actor = activeActors[i];
                    if (actor?.Bottle == null || !HasFollowingSeatLease(actor.GlassId)) continue;
                    Transform root = MotionRoot(actor);
                    compactionPoseActors.Add(actor);
                    compactionPosePositions.Add(root.position);
                    compactionPoseRotations.Add(root.rotation);
                    compactionPoseScales.Add(root.localScale);
                }
                LayoutActiveActors(false);
                for (int i = 0; i < compactionPoseActors.Count; i++)
                {
                    Transform root = MotionRoot(compactionPoseActors[i]);
                    root.SetPositionAndRotation(compactionPosePositions[i], compactionPoseRotations[i]);
                    root.localScale = compactionPoseScales[i];
                }

                bool moved = false;
                for (int i = 0; i < activeActors.Count && !moved; i++)
                    moved = SeatChanged(activeActors[i]);
                if (!moved) return;

                if (!CanAnimate || !animateEntrance || reseatDuration <= 0f)
                {
                    SnapActorsToSeat();
                    return;
                }
                float delay = YoungestDepartureAge() < CompactionDepartureDelay ? CompactionDepartureDelay : 0f;
                float duration = delay + reseatDuration;
                SeatPresentationRun run = BeginSeatPresentation(
                    Time.realtimeSinceStartupAsDouble + duration + SeatAnimationWatchdogGrace);
                if (run == null)
                {
                    SnapActorsToSeat();
                    return;
                }
                // No barrier: input, the order clock and new commands carry on while the glasses slide.
                run.Blocking = false;
                for (int i = 0; i < compactionPoseActors.Count; i++)
                    run.Excluded.Add(compactionPoseActors[i].GlassId);
                TryStartSeatPresentationRoutine(
                    run, ReseatRoutine(run, null, Vector3.zero, Vector3.one, duration, delay));
            }
            finally
            {
                compactionPoseActors.Clear();
                ...
                compactionPoseRotations.Clear();
                compactionPoseScales.Clear();
                startingCompaction = false;
            }
        }

        private void EndCompactionWithoutSnap()
        {
            SeatPresentationRun run = activeSeatPresentation;
            if (run == null || run.Blocking) return;
            keepPosesOnSeatSettle = true;
            try { TrySettleSeatPresentation(run, BsShelfSettleReason.Cancelled, true); }
            finally { keepPosesOnSeatSettle = false; }
        ...
        public bool TryBeginSynchronizationDeferral(
            ...
            TryBeginSynchronizationDeferral(owner, -1, -1, out lease);
            ...
            out BsShelfSynchronizationLease lease)
        {
            ...
            if (owner == null || controller == null || !Ready
                || !ReferenceEquals(presentedLevel, controller.CurrentLevel)
                || presentedBoardRevision != controller.BoardRevision
                || nextSynchronizationLease == long.MaxValue
                || GlobalSynchronizationDeferred)
                return false;
            bool global = sourceGlassId == -1 && targetGlassId == -1;
            if (global)
            {
                if (SynchronizationDeferred) return false;
            }
            else if (sourceGlassId < 0 || targetGlassId < 0
                || sourceGlassId == targetGlassId
                || !actorByGlassId.ContainsKey(sourceGlassId)
                || !actorByGlassId.ContainsKey(targetGlassId)
                || IsGlassSynchronizationDeferred(sourceGlassId)
                || IsGlassSynchronizationDeferred(targetGlassId))
                return false;

            lease = new BsShelfSynchronizationLease(
                new BsShelfFlowToken(++nextSynchronizationLease));
            synchronizationDeferrals.Add(new SynchronizationDeferral
            {
                Owner = owner,
                Lease = lease,
        ...
        public bool IsSynchronizationDeferredBy(
            ...
            FindSynchronizationDeferral(owner, lease) >= 0;

        internal bool BindSynchronizationLifecycle(object owner, BsShelfSynchronizationLease lease,
            BsPourOperationStateMachine lifecycle)
        {
            int index = FindSynchronizationDeferral(owner, lease);
            if (index < 0 || lifecycle == null || !lifecycle.IsActive) return false;
            SynchronizationDeferral run = synchronizationDeferrals[index];
            if (run.Lifecycle != null) return ReferenceEquals(run.Lifecycle, lifecycle);
            if (run.Global != lifecycle.IsUndo
                || run.SourceGlassId != lifecycle.SourceGlassId || run.TargetGlassId != lifecycle.TargetGlassId)
                return false;
            run.Lifecycle = lifecycle;
            return true;
        }

        public bool SetSynchronizationPersistencePending(object owner,
            BsShelfSynchronizationLease lease, bool pending)
        {
            int index = FindSynchronizationDeferral(owner, lease);
            if (index < 0) return false;
            SynchronizationDeferral run = synchronizationDeferrals[index];
            if (run.Lifecycle != null) return false;
            run.PersistencePending = pending;
            if (!pending)
                run.Deadline = Time.realtimeSinceStartupAsDouble
                    + SynchronizationDeferralWatchdog;
            return true;
        }

        private int FindSynchronizationDeferral(object owner, BsShelfSynchronizationLease lease)
        {
            if (owner == null || !lease.IsValid) return -1;
            for (int i = 0; i < synchronizationDeferrals.Count; i++)
                if (ReferenceEquals(synchronizationDeferrals[i].Owner, owner)
                    && synchronizationDeferrals[i].Lease == lease)
                    return i;
            return -1;
        }
            ...
            BsShelfSynchronizationLease lease)
        {
            int index = FindSynchronizationDeferral(owner, lease);
            if (index < 0) return false;
            synchronizationDeferrals.RemoveAt(index);
            return true;
        }
            ...
            bool forceRefresh = false)
        {
            ...
            SynchronizationDeferral run = synchronizationDeferrals[index];
            synchronizationDeferrals.RemoveAt(index);
            if (!run.Dirty && !forceRefresh) return true;
            if (!isActiveAndEnabled) return false;
            return RefreshAfterSynchronization(run);
        }

        private bool RefreshAfterSynchronization(SynchronizationDeferral run)
        {
            BartenderBoardChange change = run.LatestChange;
            if (change != null && !IsCurrentBoardChange(change)) change = null;
            if (!run.Global && controller != null && Ready
                && ReferenceEquals(presentedLevel, controller.CurrentLevel)
                && TrySynchronizeAvailableContents(controller.Board, controller.Palette,
                    controller.BoardRevision))
                return true;
            return !SynchronizationDeferred && RefreshFromController(change);
        }

        private void SettleExpiredSynchronizationDeferrals(double now)
        {
            SynchronizationDeferral expired = null;
            for (int i = synchronizationDeferrals.Count - 1; i >= 0; i--)
            {
                SynchronizationDeferral run = synchronizationDeferrals[i];
                if (run.Lifecycle != null || run.PersistencePending || now < run.Deadline) continue;
                synchronizationDeferrals.RemoveAt(i);
                expired = run;
            }
            if (expired != null && isActiveAndEnabled)
                RefreshAfterSynchronization(expired);
        }
            ...
            Action settleShelfFlow)
        {
            ExecutePresentationCleanup(
                ...
                () =>
                {
                    ...
                    seatLeases.Clear();
                    seatCatchUps.Clear();
                    try { StopSeatAnimation(reason); }
                    finally { FinishAllDepartures(); }
                },
                null);
        }
        ...
        private void HandleBoardCommitted(BartenderBoardChange change)
        {
            ...
            if (deliveryChange
                && !DeliveryMatchesBoardChange(change.DeliveryReceipt, change))
                return;
            if (deliveryChange && Ready && controller != null && controller.CurrentLevel != null)
            {
                BsBoard deliveredBoard = controller.Board;
                if (CanDetachDelivery(deliveredBoard, change.DeliveryReceipt)
                    && TrySynchronize(deliveredBoard, controller.Palette, change.Revision,
                        change.DeliveryReceipt, true, detachDelivery: true))
                {
                    for (int i = 0; i < synchronizationDeferrals.Count; i++)
                    {
                        synchronizationDeferrals[i].Dirty = true;
                        synchronizationDeferrals[i].LatestChange = change;
                    }
                    return;
                }
            }
            if (SynchronizationDeferred)
            {
                ...
                if (!GlobalSynchronizationDeferred && Ready && controller != null)
                    TrySynchronizeAvailableContents(controller.Board, controller.Palette,
                        change.Revision);
                return;
            }
            ...
            if (TrySynchronize(
                    ...
                    KeepsOtherGlassContents(change)))
                presentedBoardRevision = controller.BoardRevision;
        }

        private static bool KeepsOtherGlassContents(BartenderBoardChange change) =>
            change.Cause == BsRoundTransitionCause.PlayerDelivery
            || change.Cause == BsRoundTransitionCause.ExtraGlass
            || change.Cause == BsRoundTransitionCause.Shuffle;
        ...
        private void HandleStateChanged(BartenderLevelState state)
        {
            ...
            {
                compactionPending = false;
                if (CompactionPlaying) StopSeatAnimation();
            }
        }
        ...
        private bool TrySynchronize(
            ...
            bool keepUnchangedContents = false,
            bool detachDelivery = false)
        {
            if (snapshot == null) return Reject("Board snapshot missing.");
            if (palette == null || palette.Count == 0)
                return Reject("Palette missing.");
            if (boardRevision < 0) return Reject("Invalid board revision.");
            ...
            try
            {
                synchronized = TrySynchronizeCore(
                    snapshot, palette, deliveryReceipt, keepUnchangedContents, detachDelivery);
            }
            finally
            {
                try
                {
                    if (!synchronized && !detachDelivery) FinishAllDepartures();
                }
                finally
                {
            ...
            if (synchronized)
            {
                ...
                publishingDetachedDelivery = detachDelivery;
                try { PublishPresentationChangedSafely(); }
                finally { publishingDetachedDelivery = false; }
            }
            return synchronized;
        }

        private bool TrySynchronizeAvailableContents(BsBoard snapshot, BsPalette palette,
            int boardRevision)
        {
            if (snapshot == null || palette == null || palette.Count == 0
                || boardRevision < 0 || snapshot.Glasses.Count != activeActors.Count)
                return false;
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                RtGlass glass = snapshot.Glasses[i];
                Actor actor = activeActors[i];
                if (actor.GlassId != glass.Id || actor.Type != glass.Type
                    || actor.Bottle == null)
                    return false;
            }
            ...
            {
                RtGlass glass = snapshot.Glasses[i];
                if (IsGlassSynchronizationDeferred(glass.Id)) continue;
                Actor actor = activeActors[i];
                SetContents(actor.Bottle, glass, palette, snapshot.Delivered, true);
                SetGarnishReadinessForSnapshot(actor, snapshot, glass);
                ActivateAndRefreshActor(actor, false);
            }
            presentedBoardRevision = boardRevision;
            PublishPresentationChangedSafely();
            return true;
        ...
        private bool CanDetachDelivery(BsBoard snapshot, BartenderDeliveryReceipt receipt)
        {
            if (!CanAnimate || !Ready || controller == null || snapshot?.Glasses == null
                || receipt?.DeliveredGlass == null
                || !ReferenceEquals(presentedLevel, controller.CurrentLevel)
                || GlobalSynchronizationDeferred
                || (activeSeatPresentation != null && activeSeatPresentation.Blocking)
                || snapshot.Glasses.Count != activeActors.Count - 1
                || !actorByGlassId.TryGetValue(receipt.DeliveredGlass.Id, out Actor delivered)
                || delivered == null || delivered.Bottle == null)
                return false;
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                RtGlass glass = snapshot.Glasses[i];
                if (glass == null || glass.Id == receipt.DeliveredGlass.Id
                    || !actorByGlassId.TryGetValue(glass.Id, out Actor actor)
                    || actor == null || actor.Bottle == null || actor.Type != glass.Type)
                    return false;
            }
            return true;
        }

        private bool DetachDeliveredGlass(
            BsBoard snapshot,
            BsPalette palette,
            BartenderDeliveryReceipt deliveryReceipt,
            bool keepUnchangedContents)
        {
            if (!CanDetachDelivery(snapshot, deliveryReceipt)) return false;
            Actor deliveredActor = actorByGlassId[deliveryReceipt.DeliveredGlass.Id];
            if (activeSeatPresentation != null) EndCompactionWithoutSnap();

            activeActors.Clear();
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                RtGlass glass = snapshot.Glasses[i];
                Actor actor = actorByGlassId[glass.Id];
                activeActors.Add(actor);
                if (IsGlassSynchronizationDeferred(glass.Id)) continue;
                SetContents(actor.Bottle, glass, palette, snapshot.Delivered,
                    keepUnchangedContents
                    && Mathf.Abs(actor.Bottle.DisplayVolume - actor.Bottle.UnitCount) < 0.001f);
                SetGarnishReadinessForSnapshot(actor, snapshot, glass);
            }

            actorByGlassId.Remove(deliveredActor.GlassId);
            glassIdByBottle.Remove(deliveredActor.Bottle);
            deliveredActor.GlassId = -1;
            BeginDeparture(deliveredActor, deliveryReceipt);

            for (int i = 0; i < activeActors.Count; i++)
            {
                Actor actor = activeActors[i];
                if (!IsGlassSynchronizationDeferred(actor.GlassId))
                    ActivateAndRefreshActor(actor, false);
            }
            compactionPending = true;
            TryStartCompaction();
        ...
        {
            if (detachDelivery)
                return DetachDeliveredGlass(snapshot, palette, deliveryReceipt, keepUnchangedContents);
            StopSeatAnimation();
            compactionPending = false;
            ...
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                ...
                {
                    return Reject($"Glass {glass.Id} type changed.");
                }

                actor.Assigned = true;
                activeActors.Add(actor);
                SetContents(actor.Bottle, glass, palette, snapshot.Delivered,
                    keepUnchangedContents && !ReferenceEquals(actor, addedActor)
                    && Mathf.Abs(actor.Bottle.DisplayVolume - actor.Bottle.UnitCount) < 0.001f);
                SetGarnishReadinessForSnapshot(actor, snapshot, glass);
            }
            ...
            {
                Actor actor = actors[i];
                // A glass from an earlier delivery keeps its pool slot until its own dissolve finishes.
                if (!actor.Assigned || ContainsActor(activeActors, actor) || IsDepartingActor(actor)) continue;
                if (ReferenceEquals(actor, deliveredActor))
                {
                    actorByGlassId.Remove(actor.GlassId);
                    glassIdByBottle.Remove(actor.Bottle);
                    actor.GlassId = -1;
                    BeginDeparture(actor, deliveryReceipt);
                }
                else Release(actor);
            }
                                      ...
                                      out int rowCount, out string reason)
        {
            ...
            if (level == null)
            {
                reason = "Level asset missing.";
                return false;
            }
            if (snapshot == null)
            {
                reason = "Board snapshot missing.";
                return false;
            }
            if (palette == null || palette.Count == 0)
            {
                reason = "Palette missing.";
                return false;
            }
            if (level.ColumnsPerRow <= 0 || level.ColumnsPerRow > MaximumColumnsPerRow)
            {
                reason = $"ColumnsPerRow must be 1-{MaximumColumnsPerRow}.";
                return false;
            }
            if (snapshot.Glasses.Count > MaximumActiveGlasses)
            {
                reason = $"Level needs {snapshot.Glasses.Count} slots.";
                return false;
            }
            ...
            for (int i = 0; i < snapshot.Glasses.Count; i++)
            {
                ...
                switch (glass.Type)
                {
                    ...
                    default:
                        reason = $"Unsupported glass type: {(int)glass.Type}.";
                        return false;
                }
            }
                                    ...
                                    bool extraShotReserve, out string reason)
        {
            ...
            for (int i = 0; i < pool.Count; i++)
            {
                ...
                {
                    reason = $"'{bottle.name}' appears in multiple pools.";
                    return false;
                }
                Transform seatRoot = bottle.transform.parent;
                if (!uniqueSeatRootScratch.Add(seatRoot))
                {
                    reason = $"'{seatRoot.name}' is assigned twice.";
                    return false;
                }
                                              ...
                                              GlassType type, out string reason)
        {
            ...
            if (count < required)
            {
                reason = $"{BsRules.DisplayName(type)} needs {required} objects; {count} bound.";
                return false;
            }
            reason = null;
            return true;
                                           ...
                                           out string reason)
        {
            ...
            if (bottle == null)
            {
                reason = $"{BsRules.DisplayName(type)} pool {index} missing.";
                return false;
            }
            ...
            {
                reason = $"'{bottle.name}' profile/front missing.";
                return false;
            }
            if (bottle.profile.shelfReferenceScale <= 0f)
            {
                reason = $"'{bottle.name}' profile scale invalid.";
                return false;
            }
            ...
            {
                reason = $"'{bottle.name}' front visual mismatched.";
                return false;
            }
            int expected = BsRules.Capacity(type);
            if (bottle.profile.capacity != expected)
            {
                reason = $"'{bottle.name}' capacity must be {expected}.";
                return false;
            }
            reason = null;
            return true;
                                                ...
                                                out string reason)
        {
            ...
            {
                reason = $"Shelf rows must be {ShelfLayoutSolver.MinimumRowCount}-{ShelfLayoutSolver.MaximumRowCount}.";
                return false;
            }
            if (useOverflowLayout
                && rowCount != ShelfLayoutSolver.MaximumRowCount)
            {
                reason = "Four shelves require overflow layout.";
                return false;
            }
            ...
            if (actual != expected)
            {
                reason = $"{expected}-shelf layout needs {expected} anchors; {actual} bound.";
                return false;
            }
            ...
            for (int i = 0; i < anchors.Length; i++)
            {
                ...
                if (anchor == null)
                {
                    reason = $"{expected}-shelf anchor {i + 1} missing.";
                    return false;
                }
                if (anchor == LayoutSpace || !anchor.IsChildOf(LayoutSpace))
                {
                    reason = $"Anchor '{anchor.name}' is outside Layout Space.";
                    return false;
                }
                if (IsUnderRuntimeDrivenShelfPiece(anchor))
                {
                    reason = $"Anchor '{anchor.name}' is in a moving hierarchy.";
                    return false;
                }
                ...
                {
                    reason = $"Anchor '{anchor.name}' position invalid.";
                    return false;
                }
                if (i == 0) commonX = local.x;
                else if (Mathf.Abs(local.x - commonX) > 0.001f)
                {
                    reason = $"{expected}-shelf anchors must share center X.";
                    return false;
                }
                if (local.y >= previousY)
                {
                    reason = $"{expected}-shelf anchors must be top-to-bottom.";
                    return false;
                }
                for (int earlier = 0; earlier < i; earlier++)
                {
                    if (anchors[earlier] != anchor) continue;
                    reason = $"Anchor '{anchor.name}' is duplicated.";
                    return false;
                }
                previousY = local.y;
            }
                                           ...
                                           out string reason)
        {
            ...
            {
                reason = $"{rowCount} rows need more plank bindings.";
                return false;
            }
            ...
            {
                reason = $"{rowCount} rows need more post bindings.";
                return false;
            }
            for (int span = 0; span < spanCount; span++)
            {
                ...
                {
                    reason = $"Shelf span {span + 1} needs two posts.";
                    return false;
                }
            }
            reason = null;
        ...
        private void SetContents(LiquidBottle bottle, RtGlass glass, BsPalette palette,
                                 int delivered, bool preserveUnchanged = false)
        {
            colorScratch.Clear();
            bool wholeGlassLocked = glass.IsChained(delivered);
            int lockedRun = 0;
            int lockedPresentationMask = 0;
            for (int i = 0; i < glass.Layers.Count; i++)
            {
                Layer layer = glass.Layers[i];
                // Use only visible mechanic state to distinguish concealed bands; never leak whether
                // their real colours match. Locked takes priority when a unit is also hidden. A layer under a
                // still-locked layer stays locked until its whole lock group opens.
                bool locked = GlassLockPresenter.IsLayerPresentedLocked(glass, i, delivered);
                if (locked && !wholeGlassLocked && i < LiquidBottle.MaxBands)
                    lockedPresentationMask |= 1 << i;
                lockedRun = locked ? lockedRun + 1 : 0;
                Color color = locked
                    ? LockedLayerColor(lockedRun, wholeGlassLocked)
                    : layer.Hidden ? hiddenLayerColor : palette.ColorAt(layer.Color);
                if (wholeGlassLocked)
                    color = DimLockPresentationColor(color, chainedGlassBrightness);
                colorScratch.Add(color);
            }
            actorByGlassId.TryGetValue(glass.Id, out Actor actor);
            if (preserveUnchanged && actor != null
                && actor.AppliedContentsModelVersion == bottle.ModelVersion
                && bottle.capacity == bottle.profile.capacity
                && actor.AppliedColors.Count == colorScratch.Count)
            {
                bool same = true;
                for (int i = 0; i < colorScratch.Count && same; i++)
                    same = actor.AppliedColors[i].Equals(colorScratch[i]);
                // SetUnits clears reveal effects and changes ModelVersion; an unrelated pour must do neither.
                if (same)
                {
                    bottle.SetLockedPresentationMask(lockedPresentationMask);
                    return;
                }
            }
            bottle.capacity = bottle.profile.capacity;
            ...
            if (actor != null)
            {
                actor.AppliedColors.Clear();
                actor.AppliedColors.AddRange(colorScratch);
                actor.AppliedContentsModelVersion = bottle.ModelVersion;
            }
        }

        // Both tones stay in one navy slate family, separate from lighter mystery fill. Explicit shader
        // metadata keeps every locked unit countable even with zero lift. Neither tone nor boundary reveals
        // concealed colours. A dimmed chained glass has one whole-glass lock, so it keeps a plain fill.
        private Color LockedLayerColor(int runPosition, bool wholeGlassLocked)
        {
            if (wholeGlassLocked || runPosition % 2 == 1) return lockedLayerColor;
            return new Color(Mathf.Min(1f, lockedLayerColor.r + lockedLayerSeamLift),
                             Mathf.Min(1f, lockedLayerColor.g + lockedLayerSeamLift),
                             Mathf.Min(1f, lockedLayerColor.b + lockedLayerSeamLift),
                             lockedLayerColor.a);
        }
        ...
        private void LayoutActiveActors(bool stopAnimation = true)
        {
            ...
            for (int row = 0; row < plan.RowCount; row++)
            {
                ...
                {
                    Actor actor = SeatedActor(rowPlan.ItemStart + column);
            ...
            if (spaceGlassesByOutline) EvenShelfGaps(plan);
            for (int row = 0; row < plan.RowCount; row++)
            {
                ShelfRowLayout rowPlan = plan.RowAt(row);
            ...
            if (overflowCount > 0)
            {
                ...
                {
                    Actor actor = SeatedActor(mainCount + column);
        ...
        private void CenterRowSilhouette(int start, int count, float shelfCenterX)
        {
            ...
            {
                SpriteRenderer renderer = SeatedActor(i).PlacementRenderer;
            ...
            for (int i = start; i < start + count; i++)
            {
                Transform root = MotionRoot(SeatedActor(i));
        ...
        private void ResolveSeatOrder(int count)
        {
            seatOrderCount = count <= seatOrder.Length ? count : 0;
            for (int i = 0; i < seatOrderCount; i++)
            {
                seatOrder[i] = i;
                seatGlassIds[i] = activeActors[i].GlassId;
            }
            if (!arrangeGlassesByType || seatOrderCount == 0) return;
            ShelfArrangement.TryBuildSeatOrder(presentedLevel, seatGlassIds, seatOrderCount,
                seatOrder, arrangementWorkspace);
        }

        private Actor SeatedActor(int seat) =>
            seat < seatOrderCount && seatOrderCount == activeActors.Count
                ? activeActors[seatOrder[seat]]
                : activeActors[seat];

        private void EvenShelfGaps(ShelfLayoutPlan plan)
        {
            if (seatOrderCount != activeActors.Count) return;
            float tightest = float.PositiveInfinity;
            for (int row = 0; row < plan.RowCount; row++)
            {
                ShelfRowLayout rowPlan = plan.RowAt(row);
                float gap = MeasureEvenGap(rowPlan.ItemStart, rowPlan.ItemCount);
                rowEvenGap[row] = gap;
                if (gap > 0f && gap < tightest) tightest = gap;
            }
            if (float.IsInfinity(tightest)) return;

            float loosest = tightest * maximumShelfGapRatio;
            for (int row = 0; row < plan.RowCount; row++)
            {
                if (rowEvenGap[row] <= 0f) continue;
                ShelfRowLayout rowPlan = plan.RowAt(row);
                ApplyRowGap(rowPlan.ItemStart, rowPlan.ItemCount,
                    Mathf.Min(rowEvenGap[row], loosest));
            }
        }

        private float MeasureEvenGap(int start, int count)
        {
            if (count < 2) return 0f;
            float gaps = 0f;
            for (int seat = start; seat < start + count; seat++)
            {
                MeasureVisibleLayoutX(SeatedActor(seat),
                    out seatVisibleMinX[seat], out seatVisibleMaxX[seat]);
                if (seat > start) gaps += seatVisibleMinX[seat] - seatVisibleMaxX[seat - 1];
            }
            return gaps > 0f ? gaps / (count - 1) : 0f;
        }

        private void ApplyRowGap(int start, int count, float gap)
        {
            float previousMaxX = seatVisibleMaxX[start];
            for (int seat = start + 1; seat < start + count; seat++)
            {
                float shift = previousMaxX + gap - seatVisibleMinX[seat];
                previousMaxX = seatVisibleMaxX[seat] + shift;
                if (Mathf.Abs(shift) < 0.0001f) continue;
                Transform root = MotionRoot(SeatedActor(seat));
                Vector3 local = GlassSpace.InverseTransformPoint(root.position);
                local.x += shift;
                root.position = GlassSpace.TransformPoint(local);
            }
        ...
        private void MeasureVisibleLayoutX(Actor actor, out float minX, out float maxX)
        {
            SpriteRenderer renderer = actor.PlacementRenderer;
            Bounds bounds = renderer.sprite.bounds;
            ShelfArrangement.VisibleWidthInsets(actor.Type, out float leftShare, out float rightShare);
            float left = bounds.min.x + bounds.size.x * leftShare;
            float right = bounds.max.x - bounds.size.x * rightShare;
            minX = float.PositiveInfinity;
            maxX = float.NegativeInfinity;
            AddLayoutX(renderer.transform, left, bounds.min.y, ref minX, ref maxX);
            AddLayoutX(renderer.transform, left, bounds.max.y, ref minX, ref maxX);
            AddLayoutX(renderer.transform, right, bounds.min.y, ref minX, ref maxX);
            AddLayoutX(renderer.transform, right, bounds.max.y, ref minX, ref maxX);
        }
        ...
        private SeatPresentationRun PlayEntrance(
            BartenderLoadingOverlayPresenter cover)
        {
            if (!CanAnimate || !dropGlassesOnEntrance || activeActors.Count == 0)
                return null;
            StopSeatAnimation();
                                ...
                                ShelfSpanBinding revealedSpan)
        {
            ...
            bool blocking = animatesPurchase || revealsShelf || animatesFurniture;
            float movementDelay = !blocking && YoungestDepartureAge() < CompactionDepartureDelay
                ? CompactionDepartureDelay : 0f;
            if (movementDelay > 0f) duration = Mathf.Max(duration, movementDelay + reseatDuration);
            ...
            // Extra-glass landing and a row reveal block commands but not the order clock. A plain re-seat takes no
            // barrier.
            if (blocking && barrierController != null
                && barrierController.AcquireAnimationPresentationBarrier(run)
        ...
        private void SnapActorsToSeat()
        {
            for (int i = 0; i < activeActors.Count; i++)
            {
                ...
                if (SkipsSeatWrite(settlingSeatRun, actor)) continue;
                GameObject actorObject = actor.Bottle.gameObject;
            ...
            bool stopRoutine)
        {
            ...
            ExecutePresentationCleanup(
                ...
                () =>
                {
                    SeatPresentationRun previousSettling = settlingSeatRun;
                    settlingSeatRun = expected;
                    try { FinishSeatPresentationVisuals(); }
                    finally { settlingSeatRun = previousSettling; }
                },
        ...
        {
            FinishRemainingSeatPresentationVisuals();
        }
        ...
        private void FinishRemainingSeatPresentationVisuals()
        {
            ...
            FinishShelfReveal();
            FinishShelfTransition();
            if (!keepPosesOnSeatSettle) SnapActorsToSeat();
        }
        ...
        private void OrderActorsForEntrance()
        {
            ...
            {
                for (int seat = 0; seat < activeActors.Count; seat++)
                {
                    Actor seated = SeatedActor(seat);
                    if (seated.Row == row) entranceOrder.Add(seated);
                }
            }
        ...
        private void RefreshResponsiveShelfOffset()
        {
            ...
            if (SeatAnimationPlaying || SynchronizationDeferred || compactionPending
        ...
        {
            ClearPresentation();
            LastError = string.IsNullOrEmpty(reason) ? "Unknown presentation error." : reason;
